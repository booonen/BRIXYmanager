...
function renderNodes() {
  ...
  if (!list.length) {
    el.innerHTML = `<div class="empty-state"><div class="empty-icon">◉</div>
      <h3>${t('nodes.empty.none')}</h3><p>${t('nodes.empty.none_desc')}</p>
  ...
  el.innerHTML = `<table class="data-table"><thead><tr>
    ...
    list.map(n => {
      ...
      <td class="actions-cell">
        <button class="btn btn-sm" onclick="openNodeModal('${n.id}')">${t('common.btn.edit')}</button>
...
function showNodeDetail(id) {
  ...
  if (isPassengerStop(node)) {
    html += `<div class="mb-16"><button class="btn btn-sm" onclick="_departureStationId='${node.id}';setBoardMode('departures');switchTab('departures');document.querySelector('.content').scrollTop=0">▤ ${t('nodes.detail.view_departure_board')}</button></div>`;
  }
  ...
  if ((node.type === 'station' || node.type === 'junction' || node.type === 'waypoint') && conns.length > 0) {
    ...
    if (sch && sch.tracks && sch.tracks.length > 0) {
      ...
      html += `<div class="mt-8" style="font-size:13px"><span class="clickable" style="color:var(--accent);cursor:pointer" onclick="openSchematicEditor('${id}')">${t('railmap.btn.create_schematic')}</span></div>`;
    }
    html += `</div>`;
  }
  ...
  } else html += `<div class="text-dim mt-8" style="font-size:13px">${t('schedule.empty.no_scheduled')}</div>`;
  html += '</div></div>';
...
function openNodeModal(id, hField) {
  ...
  openModal(n ? t('nodes.modal.edit_title') : t('nodes.modal.add_title'), `
    <div class="form-row">
    ...
    <div id="plat-sec" style="${(n ? isPassengerStop(n) : (_lastNodeType==='station'||_lastNodeType==='bus_stop'))?'':'display:none'}">
      ...
      ${n && n.type === 'station' && connectedNodes(n.id).length > 0 ? `<div class="form-group mt-8"><label>${t("railmap.label.station_schematic")}</label>
    ...
    ${n && n.type === 'junction' && connectedNodes(n.id).length > 0 ? `<div class="form-group mt-8"><label>${t("railmap.label.junction_schematic")}</label>
      <button class="btn btn-sm" onclick="closeModal();setTimeout(()=>openSchematicEditor('${n.id}'),100)">${n.schematic?.tracks?.length ? 'Edit Schematic' : '+ Create Schematic'}</button>
      ${n.schematic?.tracks?.length ? `<span class="text-dim" style="font-size:12px;margin-left:8px">${n.schematic.tracks.length} track${n.schematic.tracks.length!==1?'s':''} defined</span>` : ''}
    </div>` : ''}
    ${n && n.type === 'waypoint' && connectedNodes(n.id).length > 0 ? `<div class="form-group mt-8"><label>${t("railmap.label.waypoint_schematic")}</label>
      <button class="btn btn-sm" onclick="closeModal();setTimeout(()=>openSchematicEditor('${n.id}'),100)">${n.schematic?.tracks?.length ? 'Edit Schematic' : '+ Create Schematic'}</button>
      ${n.schematic?.tracks?.length ? `<span class="text-dim" style="font-size:12px;margin-left:8px">${n.schematic.tracks.length} track${n.schematic.tracks.length!==1?'s':''} defined</span>` : ''}
    </div>` : ''}`,
    `<button class="btn" onclick="closeModal()">${t('common.btn.cancel')}</button>
     <button class="btn btn-primary" onclick="saveNode()">${n?t('common.btn.save'):t('nodes.btn.add')}</button>`);
...
function saveNode() {
  ...
  if (!name) { toast(t('common.toast.name_required'), 'error'); return; }
...
function schRenderModal(nodeId) {
  ...
  const SIDE_NAMES = { a: t('railmap.northbound'), b: t('railmap.southbound'), c: t('railmap.eastbound'), d: t('railmap.westbound') };
  const allSides = ['a', 'b', 'c', 'd'];
  ...
  // Sides table
  let sidesHtml = `<div class="mb-16"><strong style="font-size:12px;text-transform:uppercase;color:var(--text-muted)">${t('railmap.segment_directions')}</strong>
    <p class="text-dim" style="font-size:12px;margin:4px 0 8px">${t('railmap.assign_directions')}</p>
    <table class="schedule-table"><thead><tr><th>${t("railmap.th.seg_towards")}</th><th>${t("railmap.th.direction")}</th></tr></thead><tbody>`;
  for (const sid of segIds) {
  ...
  let tracksHtml = `<div class="mb-16"><strong style="font-size:12px;text-transform:uppercase;color:var(--text-muted)">${t('railmap.tracks_title')}</strong>
    <p class="text-dim" style="font-size:12px;margin:4px 0 8px">${t('railmap.track_connections')}</p>`;
  ...
  if (sch.tracks.length) {
    ...
    tracksHtml += `<div class="text-dim" style="font-size:13px">t('railmap.no_tracks')</div>`;
  }
  tracksHtml += `<button class="btn btn-sm mt-8" onclick="schAddTrack('${nodeId}')">${t('railmap.btn.add_track')}</button></div>`;

  openModal(t('railmap.modal.title', { name: node.name }), sidesHtml + tracksHtml,
    `<button class="btn" onclick="closeModal()">${t('common.btn.cancel')}</button>
     <button class="btn btn-primary" onclick="schSave('${nodeId}')">${t("railmap.save_schematic")}</button>`);
}
...
function schSave(nodeId) {
  ...
  toast(t('railmap.toast.saved'), 'success');
}
...
function renderSegments() {
  ...
  if (!list.length) {
    el.innerHTML = `<div class="empty-state"><div class="empty-icon">─</div>
      <h3>${t('segments.empty.none')}</h3><p>${t('segments.empty.none_desc')}</p>
  ...
  el.innerHTML = `<table class="data-table"><thead><tr>
    ...
    list.map(s => {
      ...
      const typeChip = ich
        ...
        : `<span class="chip" style="font-size:10px;background:var(--accent-glow);color:var(--accent)">${t('segments.type.track_short')}</span>`;
      ...
      <td class="mono">${ich ? '—' : traffic+t('common.label.per_day')}</td>
      <td class="actions-cell">
        <button class="btn btn-sm" onclick="openSegmentModal('${s.id}')">${t('common.btn.edit')}</button>
...
function showSegmentDetail(segId) {
  ...
  if (isInterchange(seg)) {
    ...
    el.innerHTML = `<div class="detail-panel">
      ...
      <div class="flex gap-8 mb-16" style="flex-wrap:wrap">
        ...
        <span class="chip">${seg.interchangeType === 'osi' ? t('segments.type.osi') : t('segments.type.isi')}</span>
      </div></div>`;
    setTimeout(() => el.scrollIntoView({ behavior: 'smooth', block: 'start' }), 200);
    return;
  }
  ...
  let html = `<div class="detail-panel">
    ...
    <button class="btn btn-sm mb-16" onclick="promptInsertWaypoint('${segId}')" title="${t('segments.tooltip.insert_waypoint')}">✂ ${t('segments.modal.insert_waypoint_title')}</button>`;
  if (segLineIds.size) {
  ...
  if (svcSet.size) {
    html += `<table class="schedule-table mt-8 mb-16"><thead><tr><th>Service</th><th>Cat.</th>${hasMultiTrack ? '<th>Track</th>' : ''}<th>${t("services.th.route")}</th><th>${t("railmap.th.direction")}</th><th>${t("schedule.th.depart")}</th></tr></thead><tbody>` +
  ...
  if (trains.length) {
    html += `<table class="schedule-table mt-8"><thead><tr><th>Enter</th><th>Exit</th><th>Cat.</th><th>Service</th>${hasMultiTrack ? '<th>Track</th>' : ''}<th>${t("railmap.th.direction")}</th><th>${t("services.th.route")}</th></tr></thead><tbody>` +
      trains.map(t => `<tr>
...
function openSegmentModal(id, hField) {
  ...
  openModal(s ? t('segments.modal.edit_title') : t('segments.modal.add_title'), `
    ...
    <div id="seg-track-fields" style="${isInterchange==='osi'||isInterchange==='isi'?'display:none':''}">
      ...
      <div class="form-group" id="seg-tracks-group">
        ...
        <button type="button" class="btn btn-sm mt-4" onclick="addSegTrackRow()">+ ${t('railmap.btn.add_track')}</button>
      </div>
    </div>
    <div id="seg-interchange-fields" style="${isInterchange==='osi'||isInterchange==='isi'?'':'display:none'}">
      <div class="form-group"><label>${t('segments.field.interchange_type')}</label>
        <div class="flex gap-4">
          <button class="btn btn-sm" id="btn-ich-osi" onclick="document.getElementById('f-sIchType').value='osi';document.getElementById('btn-ich-osi').style.cssText='background:var(--accent);border-color:var(--accent);color:#fff';document.getElementById('btn-ich-isi').style.cssText=''" style="${!isInterchange||isInterchange==='osi'?'background:var(--accent);border-color:var(--accent);color:#fff':''}">${t('segments.type.osi')}</button>
          <button class="btn btn-sm" id="btn-ich-isi" onclick="document.getElementById('f-sIchType').value='isi';document.getElementById('btn-ich-isi').style.cssText='background:var(--accent);border-color:var(--accent);color:#fff';document.getElementById('btn-ich-osi').style.cssText=''" style="${isInterchange==='isi'?'background:var(--accent);border-color:var(--accent);color:#fff':''}">${t('segments.type.isi')}</button>
        </div>
    ...
    `<button class="btn" onclick="closeModal()">${t('common.btn.cancel')}</button>
     <button class="btn btn-primary" onclick="saveSegment()">${s?t('common.btn.save'):t('segments.btn.add')}</button>`);
  if (hField) highlightField(hField);
...
async function saveSegment() {
  ...
  if (!nodeA || !nodeB) { toast(t('segments.toast.select_both_nodes'), 'error'); return; }
  if (nodeA === nodeB) { toast(t('segments.toast.nodes_must_differ'), 'error'); return; }
  ...
  if (segType === 'osi') {
    ...
    if (distance <= 0) { toast(t('segments.toast.walk_dist_positive'), 'error'); return; }
    ...
    if (editingId) { Object.assign(getSeg(editingId), obj); toast(t('segments.toast.interchange_updated'), 'success'); }
    else { data.segments.push({ id: uid(), ...obj }); toast(t('segments.toast.interchange_added'), 'success'); }
...
function promptInsertWaypoint(segId) {
  ...
  openModal(t('segments.modal.insert_waypoint_title'), `
    <p style="font-size:13px;color:var(--text-dim);margin-bottom:16px;">
      Split <strong>${esc(nodeAName)} — ${esc(nodeBName)}</strong> (${seg.distance} km) by inserting a waypoint.</p>
    <div class="form-row">
      <div class="form-group"><label>${t('segments.field.waypoint_name')}</label><input type="text" id="f-wpName" value="" placeholder="e.g. Waypoint ${esc(nodeAName)}/${esc(nodeBName)}"></div>
      <div class="form-group"><label>Ref. Code (optional)</label><input type="text" id="f-wpRef" value="" placeholder="${t('segments.placeholder.eg_wp_ref')}"></div>
    </div>
...
function insertWaypoint(segId) {
  ...
  toast(t('segments.toast.waypoint_inserted', { name }), 'success');
...
function renderLines() {
  ...
  if (!list.length && !data.serviceGroups.length) {
    el.innerHTML = `<div class="empty-state"><div class="empty-icon">≡</div><h3>${t('lines.empty.none')}</h3>
  ...
  el.innerHTML = `<table class="data-table"><thead><tr>${sortableHeader('lines','name',t('services.th.line'))}<th>${t("lines.th.color")}</th>${sortableHeader('lines','services',t('lines.th.services'))}${sortableHeader('lines','departures',t('lines.th.departures'))}${sortableHeader('lines','stations',t('lines.th.stations'))}${sortableHeader('lines','segments',t('lines.th.segments'))}<th></th></tr></thead><tbody>` +
    list.map(g => {
      ...
      return `<tr data-id="${g.id}">
        ...
        <td class="actions-cell">
          <button class="btn btn-sm" onclick="openLineModal('${g.id}')">${t('common.btn.edit')}</button>
...
function openLineModal(id) {
  ...
  openModal(g ? t('lines.modal.edit_title') : t('lines.modal.add_title'), `
    <div class="form-row">
      <div class="form-group"><label>${t('common.field.name')}</label><input type="text" id="f-lN" value="${esc(g?.name||'')}" placeholder="${t('lines.placeholder.eg_name')}"></div>
      <div class="form-group"><label>${t('lines.field.color')}</label><input type="text" id="f-lC" value="${esc(lineColor)}" oninput="document.querySelectorAll('#f-lC-palette .color-swatch').forEach(s=>s.classList.toggle('active',s.title.startsWith(this.value)))">
...
function renderCategories() {
  ...
  if (!data.categories.length) {
    el.innerHTML = `<div class="empty-state"><div class="empty-icon">◆</div><h3>${t('modes.empty.none')}</h3>
  ...
  el.innerHTML = `<table class="data-table"><thead><tr>${sortableHeader('modes','name',t('common.th.name'))}<th>${t('modes.th.abbreviation')}</th>${sortableHeader('modes','dwell',t('modes.th.default_dwell'))}${sortableHeader('modes','clearance',t('modes.th.plt_clearance'))}${sortableHeader('modes','priority',t('modes.th.priority'))}<th></th></tr></thead><tbody>` +
    modeList.map(c => `<tr>
      ...
      <td class="actions-cell">
        <button class="btn btn-sm" onclick="openCategoryModal('${c.id}')">${t('common.btn.edit')}</button>
        <button class="btn btn-sm btn-danger" onclick="deleteCat('${c.id}')">✕</button></td>
    </tr>`).join('') + '</tbody></table>';
}
function openCategoryModal(id) {
  editingId = id || null; const c = id ? getCat(id) : null;
  openModal(c ? t('modes.modal.edit_title') : t('modes.modal.add_title'), `
    <div class="form-row">
      <div class="form-group"><label>${t('common.field.name')}</label><input type="text" id="f-cN" value="${esc(c?.name||'')}" placeholder="${t('modes.placeholder.eg_name')}"></div>
    ...
    <div class="form-group"><label>${t('modes.field.infrastructure_type')}</label><select id="f-cIT">
    ...
    `<button class="btn" onclick="closeModal()">${t('common.btn.cancel')}</button>
     <button class="btn btn-primary" onclick="saveCat()">${c?t('common.btn.save'):t('modes.btn.add')}</button>`);
}
function saveCat() {
...
function renderStock() {
  ...
  if (!data.rollingStock.length) {
    el.innerHTML = `<div class="empty-state"><div class="empty-icon">🚃</div><h3>${t('stock.empty.none')}</h3>
  ...
  el.innerHTML = `<table class="data-table"><thead><tr>
    ...
    stockList.map(s => `<tr>
      ...
      <td class="actions-cell">
        <button class="btn btn-sm" onclick="openStockModal('${s.id}')">${t('common.btn.edit')}</button>
...
function openStockModal(id) {
  ...
  openModal(s ? t('stock.modal.edit_title') : t('stock.modal.add_title'), `
    <div class="form-row">
    ...
    <div class="form-row-3">
      ...
      <div class="form-group"><label>${t('stock.field.default_dwell')}</label>
    ...
    `<button class="btn" onclick="closeModal()">${t('common.btn.cancel')}</button>
     <button class="btn btn-primary" onclick="saveStock()">${s?t('common.btn.save'):t('stock.btn.add')}</button>`);
}
...
function saveStock() {
  ...
  if (acceleration <= 0) { toast(t('modes.toast.accel_positive'), 'error'); return; }
...
function renderStockModeMatrix() {
    ...
    for (const cat of data.categories) {
      ...
      html += `<td style="text-align:center;padding:6px 4px">
        <select class="matrix-sel" data-stock="${stock.id}" data-mode="${cat.id}"
          ...
          <option value="typical" ${val==='typical'?'selected':''}>✓ ${t('common.typical')}</option>
          <option value="atypical" ${val==='atypical'?'selected':''}>~ ${t('common.atypical')}</option>
          <option value="disallowed" ${val==='disallowed'?'selected':''}>✕ ${t('common.disallowed')}</option>
        </select></td>`;
    }
    html += '</tr>';
  }
  ...
  html += `<div class="flex gap-8 mt-8" style="flex-wrap:wrap;font-size:12px;color:var(--text-dim)">
    <span><strong style="color:var(--success)">✓ ${t('common.typical')}</strong> — ${t('stock.matrix.typical_desc')}</span>
    <span><strong style="color:var(--warn)">~ ${t('common.atypical')}</strong> — ${t('stock.matrix.atypical_desc')}</span>
    <span><strong style="color:var(--danger)">✕ ${t('common.disallowed')}</strong> — ${t('stock.matrix.disallowed_desc')}</span>
  </div></div>`;
...
function renderServices() {
  ...
  if (!list.length && !data.services.length) {
    el.innerHTML = `<div class="empty-state"><div class="empty-icon">▷</div><h3>${t('services.empty.none')}</h3>
  ...
  // Build table
  html += `<table class="data-table"><thead><tr>${sortableHeader('services','name',t('schedule.th.service'))}<th>${t("services.th.line")}</th><th>${t("services.th.mode")}</th><th>${t("services.th.stock")}</th>${sortableHeader('services','stops',t('services.th.stops'))}<th>${t("services.th.route")}</th>${sortableHeader('services','departures',t('lines.th.departures'))}<th>${t('services.th.pattern')}</th><th></th></tr></thead><tbody>`;
...
function openGroupModal(id) {
  ...
  const g = id ? getGroup(id) : null;
  const lineColor = g?.color || '#8899aa';
  openModal(g ? t('lines.modal.edit_title') : t('lines.modal.new_title'), `
    <div class="form-row">
      <div class="form-group"><label>${t('lines.field.name')}</label>
        <input type="text" id="f-grpN" value="${esc(g?.name||'')}" placeholder="${t('lines.placeholder.eg_name')}"></div>
      <div class="form-group"><label>${t('lines.field.color')}</label><input type="text" id="f-grpC" value="${esc(lineColor)}" oninput="document.querySelectorAll('#f-grpC-palette .color-swatch').forEach(s=>s.classList.toggle('active',s.title.startsWith(this.value)))">
...
function quickCreateGroup() {
  ...
  div.innerHTML = `<input type="text" id="f-inlineGrp" placeholder="${t('lines.placeholder.eg_name_short')}" style="flex:1;font-size:12px;padding:5px 8px">
...
function showServiceDetail(svcId) {
  ...
  html += `<div class="flex items-center" style="justify-content:space-between"><strong style="font-size:12px;text-transform:uppercase;color:var(--text-muted);letter-spacing:0.04em">${t('lines.th.departures')} (${deps.length})</strong>
    ${deps.length ? `<button class="btn btn-sm" onclick="recalcSvcAndRefresh('${svcId}')">↻ ${t('schedule.btn.recalculate')}</button>` : ''}</div>`;
  if (deps.length) {
    ...
    for (const dep of deps) {
      ...
      html += `<td class="actions-cell">
        <button class="btn btn-sm" onclick="openDepEditModal('${dep.id}')">${t('common.btn.edit')}</button>
...
function openServiceModal(id, hField) {
  ...
  openModal(s ? t('services.modal.edit_title') : t('services.modal.add_title'), `
    <div class="form-row">
      <div class="form-group"><label>${t('services.field.name')}</label><input type="text" id="f-svN" value="${esc(s?.name||'')}" placeholder="${t('services.placeholder.eg_name')}"></div>
    ...
    <div class="form-row">
      ...
      <div class="form-group"><label>${t('services.field.line')}</label>
        <div class="flex gap-8 items-center">
          <select id="f-svG" style="flex:1"><option value="">${t('common.none_default')}</option>${grpOpts}</select>
    ...
    `<button class="btn" onclick="closeModal()">${t('common.btn.cancel')}</button>
     <button class="btn btn-primary" onclick="saveSvc()">${s?t('common.btn.save'):t('services.btn.add')}</button>`);
...
function rebuildStockOpts() {
  ...
  let html = `<option value="">${t('common.none_default')}</option>`;
...
function renderRouteBuilder() {
  ...
  if (!stops.length) {
    ...
    addContainer.innerHTML = `<div class="form-group"><label>${t('services.field.starting_node')}</label>
      <select onchange="addRouteStop(this.value);this.value='';">
        <option value="">${t('railmap.label.pick_start_node')}</option>
  ...
  for (let i = 0; i < stops.length; i++) {
    ...
    html += `<div class="route-step">
      ...
      <div class="step-body">
        ...
        ${isStation ? `<div class="step-controls mt-8">
          <select onchange="window._routeStops[${i}].platformId=this.value" style="width:120px">
            <option value="">${t('departures.label.platf')}</option>${platOpts}</select>
      ...
      ${(() => {
        ...
        if (isFirst && stops.length <= 1) return `<button class="remove-step" onclick="clearRoute()" title="${t('services.tooltip.clear_route')}">✕</button>`;
        if (isFirst) return `<button class="remove-step" onclick="removeRouteStopFront()" title="${t('services.tooltip.remove_start')}">✕</button>`;
        if (isLast) return `<button class="remove-step" onclick="removeRouteStopEnd()" title="${t('services.tooltip.remove_end')}">✕</button>`;
        return `<button class="remove-step" onclick="passRouteStop(${i})" title="${t('services.tooltip.mark_pass_through')}">✕</button>`;
      })()}
    </div>`;
    ...
    if (i < stops.length - 1) {
      ...
      if (seg) {
        ...
        if (trkArr.length <= 1) {
          ...
          trackLabel += `<option value="" ${!nextStop.trackId ? 'selected' : ''}>${t('segments.field.no_track')}</option>`;
          for (const tk of trkArr) {
  ...
  if (connected.length) {
    addContainer.innerHTML = `<select onchange="addRouteStop(this.value);this.value='';" style="width:100%">
      <option value="">${t('services.route.extend_to')}</option>
