...
function initJourneyPlanner() {
  if (!document.getElementById('np-jpOrigin')) {
    ...
    createNodePicker({
      ...
      placeholder: t('journey.placeholder.origin'),
    ...
    createNodePicker({
      ...
      placeholder: t('journey.placeholder.destination'),
      filterFn: jpFilter, displayNameFn: nodeDisplayName,
...
function jpSearch() {
  ...
  el.innerHTML = `<div class="text-dim" style="padding:20px;text-align:center">${t('journey.searching')}</div>`;
  setTimeout(() => {
...
function jpRenderResults(visible, hasPrev, hasNext, start, total) {
  ...
  let html = `<div class="mt-16" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
    ...
    <div class="flex gap-8">
      ${hasPrev ? `<button class="btn btn-sm" onclick="jpPageEarlier()">← ${t('common.earlier')}</button>` : ''}
      ${hasNext ? `<button class="btn btn-sm" onclick="jpPageLater()">${t('common.later')} →</button>` : ''}
    </div>
  </div>`;
