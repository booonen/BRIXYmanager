...
function openScheduleModal(svcId) {
  ...
  if (!svc || svc.stops.length < 2) { toast(t('services.toast.needs_stops'), 'error'); return; }
  editingId = null;
  ...
  openModal(t('schedule.modal.schedule', { name: svc.name }), `
    ...
    <div class="form-group" style="margin-bottom:8px">
      <label style="display:flex;align-items:center;gap:8px;text-transform:none;font-weight:400;font-size:13px;color:var(--text);">
        <input type="checkbox" id="sh-clear" checked onchange="refreshScheduleModal()">${t('schedule.label.clear_existing')}</label></div>
    <div id="helper-timeline"></div>
    <div class="htabs">
      <div class="htab active" id="htab-freq" onclick="switchScheduleTab('freq')">${t('schedule.sch.frequency')}</div>
      <div class="htab" id="htab-explicit" onclick="switchScheduleTab('explicit')">${t('schedule.sch.explicit')}</div>
    </div>
    ...
    `<button class="btn" onclick="closeModal()">${t('common.cancel')}</button>
     <button class="btn btn-primary" id="sched-apply-btn" onclick="applyScheduleModal('${svcId}')">${t('common.apply')}</button>`);
...
function refreshScheduleModal() {
  ...
  // Timeline bar
  let timelineHtml = `<strong style="font-size:12px;text-transform:uppercase;color:var(--text-muted);letter-spacing:0.04em">${t('schedule.label.availability_timeline')}</strong>`;
  ...
  timelineHtml += `<div style="font-size:11px;color:var(--text-muted);display:flex;gap:12px;margin-top:4px">
    <span><span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:var(--success);vertical-align:middle;margin-right:3px"></span>${t('schedule.sch.free')}</span>
    <span><span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:var(--warn);vertical-align:middle;margin-right:3px"></span>${t('schedule.sch.partial')}</span>
    <span><span style="display:inline-block;width:10px;height:10px;border-radius:2px;background:var(--danger);vertical-align:middle;margin-right:3px"></span>${t('schedule.sch.blocked')}</span>
  </div>`;
...
function updateHelperSuggestions(svcId) {
  ...
  let html = `<strong style="font-size:12px;text-transform:uppercase;color:var(--text-muted);letter-spacing:0.04em">${t('schedule.sch.suggested_schedules')}</strong>`;
  html += `<table class="schedule-table mt-8"><thead><tr><th>${t('schedule.th.frequency')}</th><th>${t('schedule.th.deps')}</th><th>${t('schedule.th.best_start')}</th><th>${t('schedule.th.conflicts')}</th><th></th></tr></thead><tbody>`;
  ...
  for (const freq of freqs) {
    ...
    const statusText = bestConflicts === 0 ? `✓ ${t('schedule.sch.clear')}` : (bestConflicts === 1 ? t('schedule.sch.conflicts_n_one') : t('schedule.sch.conflicts_n_other', { n: bestConflicts }));

    html += `<tr>
      <td class="mono" style="font-weight:500">${t('schedule.sch.every_n_min', { n: freq })}</td>
      ...
      <td class="actions-cell"><button class="btn btn-sm btn-primary" onclick="applyFrequencySchedule('${svcId}',${firstDep},${effectiveEnd},${freq})">${t('common.apply')}</button></td>
    </tr>`;
  }
  ...
  html += `<tr style="border-top:1px solid var(--border)">
    <td><input type="number" id="sh-custom-freq" min="1" max="1440" value="${esc(customFreqVal)}" placeholder="${t('schedule.sch.custom_freq_placeholder')}" onchange="refreshScheduleModal()" style="width:60px;font-size:12px;font-family:var(--font-mono)">m</td>`;
  if (hasCustom) {
    ...
    html += `<td class="mono">${bestCount}</td>
      <td class="mono" style="color:var(--warn)">${toTime(firstDep)}</td>
      <td style="color:${statusColor};font-weight:500">${statusText}</td>
      <td class="actions-cell"><button class="btn btn-sm btn-primary" onclick="applyFrequencySchedule('${svcId}',${firstDep},${effectiveEnd},${customFreq})">${t('common.apply')}</button></td>`;
  } else {
...
function applyFrequencySchedule(svcId, firstDep, endTime, freq) {
  ...
  const action = clearExisting ? t('schedule.btn.replace') : t('schedule.btn.add_departures');
...
function showTrainSchedule(depId) {
  ...
  openModal(`${svc.name}`, `
    ...
    `<button class="btn" onclick="closeModal()">${t('common.close')}</button>
     <button class="btn" onclick="closeModal();openDepEditModal('${depId}')">${t('services.btn.edit_times')}</button>`);
}
  ...
  let stopsHtml = dep.times.map((t, i) => {
    ...
    return `<div style="display:grid;grid-template-columns:24px 1fr 90px 90px 100px 80px 50px;align-items:center;gap:4px;padding:6px 0;border-bottom:1px solid var(--border);font-size:13px" data-dep-idx="${i}">
      ...
      <div>${(!isFirst && !isLast && isStation) ? `<label style="font-size:11px;color:var(--text-dim);display:flex;align-items:center;gap:3px;cursor:pointer" title="Skip this stop (creates variant service)">
        <input type="checkbox" class="dep-skip-cb" data-idx="${i}" ${stop?.passThrough?'checked':''}>${t('schedule.dep_edit.col_skip')}</label>` : ''}</div>
    </div>`;
  }).join('');

  openModal(t('schedule.modal.edit_dep', { name: svc.name + ' @ ' + toTime(dep.startTime) }), `
    <div class="flex gap-8 items-center mb-16" style="flex-wrap:wrap">
      <div class="form-group" style="margin-bottom:0"><label>${t('services.field.stock_override')}</label>
        <select id="dep-stock-override" style="font-size:12px;padding:5px 8px;width:200px">
          <option value="">${t('schedule.dep_edit.use_default_stock')}${svc.stockId && getStock(svc.stockId) ? ' ('+esc(getStock(svc.stockId).name)+')' : ''}</option>
    ...
    <p style="font-size:13px;color:var(--text-dim);margin-bottom:12px">
      ${t('schedule.dep_edit.adjust_desc')}</p>
    <div style="display:grid;grid-template-columns:24px 1fr 90px 90px 100px 80px 50px;gap:4px;padding:4px 0;border-bottom:1px solid var(--border);margin-bottom:4px">
      <div></div><div class="text-muted" style="font-size:10px;text-transform:uppercase">${t('schedule.dep_edit.col_station')}</div>
      <div class="text-muted" style="font-size:10px;text-transform:uppercase;text-align:right">${t('schedule.dep_edit.col_arrive')}</div>
      <div class="text-muted" style="font-size:10px;text-transform:uppercase;text-align:right">${t('schedule.dep_edit.col_depart')}</div>
      <div class="text-muted" style="font-size:10px;text-transform:uppercase">${t('schedule.dep_edit.col_platform')}</div>
      <div class="text-muted" style="font-size:10px;text-transform:uppercase">${t('schedule.dep_edit.col_dwell')}</div>
      <div></div>
    </div>
    <div id="dep-edit-stops">${stopsHtml}</div>
    <p class="text-dim mt-8" style="font-size:11px">${t('schedule.dep_edit.skip_desc')}</p>`,
    `<button class="btn" onclick="closeModal()">${t('common.cancel')}</button>
     <button class="btn btn-primary" onclick="saveDepEdit('${depId}')">${t('schedule.dep_edit.save_cascade')}</button>`);
}
...
function renderSchedule() {
  ...
  c.innerHTML = `<div class="form-group" style="max-width:300px;"><label>${t('services.th.filter_service')}</label>
    <select id="sch-filter" onchange="renderSchTable()"><option value="">${t('common.all_services')}</option>
    ${data.services.map(s => `<option value="${s.id}">${esc(s.name)}</option>`).join('')}
    ${orphanCount ? `<option value="__orphaned__">${t('common.orphaned_deps', { n: orphanCount })}</option>` : ''}
    </select></div>
...
function renderSchTable() {
  ...
  if (!deps.length) {
    el.innerHTML = `<div class="text-dim mt-16" style="font-size:13px;">${t('departures.empty.no_departures')}</div>`;
    return;
  }
  ...
  if (isOrphan) {
    el.innerHTML = `<p style="font-size:13px;color:var(--warn);margin-bottom:12px">These departures reference services that no longer exist.</p>
      ...
      <table class="schedule-table"><thead><tr><th>${t('departures.th.depart')}</th><th>${t('services.th.service_id')}</th><th>${t('services.th.stops')}</th><th></th></tr></thead><tbody>` +
      deps.map(d => {
  ...
  el.innerHTML = `<table class="schedule-table"><thead><tr><th>${t('departures.th.depart')}</th><th>${t('services.th.service')}</th><th>${t('services.th.pattern')}</th><th>${t('services.th.route')}</th><th>${t('departures.th.arrival')}</th><th></th></tr></thead><tbody>` +
    deps.map(d => {
