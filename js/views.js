...
function runIssueDetection() {
  ...
  // Schedule pattern with no possible run dates: pattern.days = [] and no specificDates (or all are excluded)
  for (const svc of data.services) {
    const pat = svc.schedulePattern;
    if (!pat) continue;
    if (!pat.days || pat.days.length !== 0) continue;
    const excl = pat.excludeDates || [];
    const specifics = (pat.specificDates || []).filter(d => !excl.includes(d));
    if (specifics.length > 0) continue;
    issues.push({ severity: 'medium', type: t('issue.type.schedule_no_run_dates'), typeKey: 'schedule_no_run_dates',
      desc: t('issue.desc.schedule_no_run_dates', { name: svc.name }),
      detail: t('issue.detail.schedule_no_run_dates'),
  ...
  // Suspicious dwell length: >15 min at non-terminal non-pass-through stops (likely data entry error)
  for (const svc of data.services) {
    ...
    for (let i = 1; i < svc.stops.length - 1; i++) {
      const stop = svc.stops[i];
      if (stop.passThrough) continue;
      const dwell = stop.dwell ?? defDwell;
      if (dwell > 900) {
        const node = getNode(stop.nodeId);
        issues.push({ severity: 'medium', type: t('issue.type.suspicious_dwell'), typeKey: 'suspicious_dwell',
          desc: t('issue.desc.suspicious_dwell', { name: svc.name, station: node?.name || '?', mins: Math.round(dwell / 60) }),
          detail: t('issue.detail.suspicious_dwell'),
          action: `openServiceModal('${svc.id}')` });
        break;
      }
    }
  ...
  for (const n of data.nodes) {
    ...
    if (n.type === 'junction') {
      const conns = connectedNodes(n.id).length;
      if (conns === 2) {
        issues.push({ severity: 'low', type: t('issue.type.junction_2_connections'), typeKey: 'junction_2_connections',
          desc: t('issue.desc.junction_2_connections', { name: n.name }),
          detail: t('issue.detail.junction_2_connections'),
          action: `setSearchValue('node-search','');switchTab('nodes');showNodeDetail('${n.id}')` });
      }
    }
...
function placeLabels(labelData) {
  ...
  // Score labels by THI so junctions and termini win over busy-corridor through-stops.
  // Falls back to the old daily-traffic count if THI isn't available (defensive).
  const thiMap = (typeof thiByDisplayName === 'function') ? thiByDisplayName() : new Map();
  const scored = labelData.map(ld => ({
    ...ld,
    score: thiMap.get(ld.dn)?.thi || 0,
...
function detailMapDestroy(containerId) {
  ...
  if (!entry) return;
  if (entry.leaflet) { try { entry.leaflet.remove(); } catch(e) { console.warn('detail map cleanup failed', e); } }
...
function _relRenderStep(n) {
  ...
  if (n === 1) {
    ...
    if (c.createServices) {
      ...
      for (const ln of (data.serviceGroups || [])) {
