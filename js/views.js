  ...
  el.innerHTML = `
    ...
    <div id="stab-general" class="settings-section">
      ...
      <div class="form-group" style="margin-top:24px">
        <button class="btn btn-danger" onclick="newSystem()">${t('ie.btn.new_system')}</button>
    ...
    <div id="stab-issues" class="settings-section" style="display:none">
      ...
      <div style="border-top:1px solid var(--border);padding-top:12px">
        <strong style="font-size:11px;text-transform:uppercase;color:var(--text-muted);letter-spacing:0.04em">${t('issues.individual_types')}</strong>
    ...
    if (segs.length > 1) {
      ...
      if (allSameOrEmpty) {
        issues.push({ severity: 'medium', type: t('issues.type.duplicate_segment'), typeKey: 'duplicate_segment',
          desc: t('issues.desc.duplicate_segment', { n: segs.length, type: segs[0].interchangeType ? segs[0].interchangeType.toUpperCase() : t('segments.seg.type_track_display'), from: nodeName(segs[0].nodeA), to: nodeName(segs[0].nodeB) }),
    ...
    for (let i = 0; i < overlapGeoSegs.length; i++) {
      ...
      for (let j = i + 1; j < overlapGeoSegs.length; j++) {
        ...
        if (overlapKm >= MIN_OVERLAP) {
          ...
          const descKey = sharedEndpoint ? 'issues.desc.overlap_branching' : 'issues.desc.overlap_mid';
  ...
  if (verifiedSegs.length) {
    html += `<div style="margin-top:16px;border-top:1px solid var(--border);padding-top:12px">
      <div class="clickable" style="font-size:12px;color:var(--text-muted);cursor:pointer" onclick="const el=document.getElementById('verified-segs-list');el.style.display=el.style.display==='none'?'':'none'">
        ${t('issues.verified_summary', { n: verifiedSegs.length })} · <span style="color:var(--accent)">${t('issues.hidden.show')} / ${t('issues.hidden.hide')}</span>
      </div>
        ...
        ${verifiedSegs.map(seg => {
          ...
          return `<div class="issue-item severity-low" style="cursor:pointer" onclick="switchTab('segments');showSegmentDetail('${seg.id}')">
            <div style="display:flex;align-items:center;justify-content:space-between">
              ...
              <button class="btn btn-sm" onclick="event.stopPropagation();unverifySegment('${seg.id}')">${t('issues.unverify_btn')}</button>
            </div>
          </div>`;
        }).join('')}
      </div>
...
function mapFitBounds() {
  ...
  if (coords.length === 0) { toast(t('geomap.toast.no_coords'), 'error'); return; }
...
function renderImportExport() {
  ...
  el.innerHTML = `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;max-width:800px">
      <div class="ie-card">
        <h3>${t('ie.json.title')}</h3>
        <p class="text-dim" style="font-size:13px;margin:8px 0 16px">${t('ie.json.desc')}</p>
        <div class="flex gap-8">
          <button class="btn" onclick="importData()">↑ ${t('ie.btn.import_json')}</button>
          <button class="btn" onclick="exportData()">↓ ${t('ie.btn.export_json')}</button>
        </div>
      </div>
      <div class="ie-card">
        <h3>${t('ie.ogf.title')}</h3>
        <p class="text-dim" style="font-size:13px;margin:8px 0 16px">${t('ie.ogf.desc')}</p>
        <button class="btn" onclick="startRelationImport()">${t('ie.ogf.btn')}</button>
      </div>
      <div class="ie-card">
        <h3>${t('ie.csv_nodes.title')}</h3>
        <p class="text-dim" style="font-size:13px;margin:8px 0 16px">${t('ie.csv_nodes.desc')}</p>
        <button class="btn" onclick="startCSVNodeImport()">${t('ie.csv_btn')}</button>
      </div>
      <div class="ie-card">
        <h3>${t('ie.csv_segs.title')}</h3>
        <p class="text-dim" style="font-size:13px;margin:8px 0 16px">${t('ie.csv_segs.desc')}</p>
        <button class="btn" onclick="startCSVSegmentImport()">${t('ie.csv_btn')}</button>
      </div>
      <div class="ie-card">
        <h3>${t('ie.saves.title')}</h3>
        <p class="text-dim" style="font-size:13px;margin:8px 0 16px">${t('ie.saves.desc')}</p>
        <button class="btn" onclick="openSaveManager()">${t('ie.saves.manage')}</button>
      </div>
    </div>`;
}
...
function _relRenderStep(n) {
  ...
  const header = (step, title) => {
    return `<div class="csv-wizard-header">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
        <div>
          <h2 style="font-family:var(--font-display);font-size:22px;font-weight:600;margin-bottom:2px">${t('ie.rel.title')}</h2>
          <div class="text-dim" style="font-size:13px">${t('ie.csv.step_of', { n: step, total: totalSteps })}: ${title}</div>
        </div>
        <button class="btn" onclick="_relCancel()">${t('common.cancel')}</button>
      </div>
  ...
  if (n === 1) {
    ...
    let svcHtml = `<div class="form-group" style="margin-top:16px;padding-top:16px;border-top:1px solid var(--border)">
      <label style="display:flex;align-items:center;gap:6px;cursor:pointer">
        ...
        ${t('ie.rel.create_services')}</label>
      <p class="text-dim" style="font-size:12px;margin-top:4px">${t('ie.rel.create_services_desc')}</p>
    </div>`;
    if (c.createServices) {
      let catOpts = `<option value="">${t('common.select')}</option>`;
      for (const cat of data.categories) {
        catOpts += `<option value="${cat.id}" ${c.serviceCategoryId === cat.id ? 'selected' : ''}>${esc(cat.name)}</option>`;
      }
      let stockOpts = `<option value="">${t('common.none')}</option>`;
      for (const st of (data.stock || [])) {
        stockOpts += `<option value="${st.id}" ${c.serviceStockId === st.id ? 'selected' : ''}>${esc(st.name)}</option>`;
      }
      let lineOpts = `<option value="">${t('common.none')}</option>`;
      ...
      svcHtml += `<div style="margin-left:24px;margin-top:8px">
        <div class="form-group"><label>${t('ie.rel.service_mode')}</label>
          <select onchange="_relImportState.config.serviceCategoryId=this.value">${catOpts}</select></div>
        <div class="form-row">
          <div class="form-group"><label>${t('ie.rel.service_stock')}</label>
            <select onchange="_relImportState.config.serviceStockId=this.value">${stockOpts}</select></div>
          <div class="form-group"><label>${t('ie.rel.service_line')}</label>
            <select onchange="_relImportState.config.serviceLineId=this.value">${lineOpts}</select></div>
        </div>
      </div>`;
    }

    el.innerHTML = header(1, t('ie.rel.step_config')) + `
      <div style="margin-top:20px;max-width:500px">
        <div class="form-group"><label>${t('ie.rel.relation_ids')}</label>
          <p class="text-dim" style="font-size:12px;margin-bottom:6px">${t('ie.rel.relation_ids_desc')}</p>
          <textarea id="rel-ids" rows="3" placeholder="${t('ie.rel.relation_ids_placeholder')}"
        ...
        <div class="form-row">
          <div class="form-group"><label>${t('ie.csv.default_speed')}</label>
          ...
          <div class="form-group"><label>${t('ie.csv.default_tracks')}</label>
        ...
        <div class="form-group"><label>${t('ie.csv.default_platforms')}</label>
        ...
        <div class="form-group"><label>${t('ie.rel.maxspeed_boundary')}</label>
          <select onchange="_relImportState.config.maxspeedBoundary=this.value">
            <option value="default" ${c.maxspeedBoundary === 'default' ? 'selected' : ''}>${t('ie.rel.maxspeed_default')}</option>
            <option value="waypoints" ${c.maxspeedBoundary === 'waypoints' ? 'selected' : ''}>${t('ie.rel.maxspeed_waypoints')}</option>
          </select></div>
        ${modesHtml}
        ${svcHtml}
        <button class="btn btn-primary" onclick="_relFetchAndProcess()" id="rel-fetch-btn" style="margin-top:12px">${t('ie.rel.fetch_btn')}</button>
      </div>`;
    return;
  }
  ...
  if (n === 2) {
    ...
    let table = `<table class="data-table csv-preview-table"><thead><tr>
      ...
      <th>${t('ie.rel.snap_dist')}</th><th></th></tr></thead><tbody>`;
    let lastRelIdx = -1;
    for (let i = 0; i < stations.length; i++) {
      ...
      const dupLabel = st._dupType === 'ogf' ? t('ie.csv.warn_dup_ogf_existing', { name: st._dupExistingName })
        : st._dupType === 'batch' ? t('ie.csv.warn_dup_ogf')
        : st._dupType === 'cross' ? t('ie.rel.dup_cross_relation', { name: st._dupExistingName })
        : st._dupType === 'proximity' ? t('ie.rel.dup_proximity', { name: st._dupExistingName }) : '';
      table += `<tr${dupWarn}>
    ...
    el.innerHTML = header(2, t('ie.rel.step_stations')) + `
      <div style="margin-top:20px">
        <div class="text-dim" style="margin-bottom:8px">${t('ie.rel.station_count', { n: count })}</div>
        <div style="overflow-x:auto;max-height:400px;overflow-y:auto">${table}</div>
        <div class="flex gap-8" style="margin-top:16px">
          <button class="btn" onclick="_relRenderStep(1)">${t('common.back')}</button>
          <button class="btn btn-primary" onclick="_relAdvanceFromStations()">${t('common.next')}</button>
        </div>
      </div>`;
    return;
  }
  ...
  if (n === 3) {
    ...
    let table = `<table class="data-table csv-preview-table"><thead><tr>
      ...
      <th>${t('segments.field.segment_type')}</th><th>${t('segments.field.distance')}</th><th>${t('segments.field.max_speed')}</th><th></th>
    </tr></thead><tbody>`;
    let lastRelIdx = -1;
    for (let i = 0; i < segs.length; i++) {
      ...
      const segTypeLabel = seg.interchangeType === 'road' ? t('ie.rel.seg_road') : t('ie.rel.seg_track');
      table += `<tr${dupWarn}>
        ...
        <td class="text-dim" style="font-size:11px">${seg._dupType ? t('ie.csv.warn_dup_existing') : ''}</td>
      </tr>`;
    }
    table += `</tbody></table>`;
    const count = segs.filter(sg => sg._include !== false).length;
    el.innerHTML = header(3, t('ie.rel.step_segments')) + `
      <div style="margin-top:20px">
        <div class="text-dim" style="margin-bottom:8px">${t('ie.csv.review_count', { n: count })}</div>
        <div style="overflow-x:auto;max-height:400px;overflow-y:auto">${table}</div>
        <div class="flex gap-8" style="margin-top:16px">
          <button class="btn" onclick="_relRenderStep(2)">${t('common.back')}</button>
          <button class="btn btn-primary" onclick="_relRenderStep(4)">${t('common.next')}</button>
        </div>
      </div>`;
    return;
  }
  ...
  if (n === 4) {
    ...
    if (!s.config.createServices) {
      html += `<p class="text-dim">${t('ie.rel.services_disabled')}</p>`;
    } else {
      _relBuildServices();
      html += `<p class="text-dim" style="margin-bottom:12px">${t('ie.rel.services_preview_desc')}</p>`;
      for (let i = 0; i < s.services.length; i++) {
        ...
        html += `<div class="ie-card" style="margin-bottom:12px;padding:12px">
          ...
          <div class="text-dim" style="font-size:12px">${t('ie.rel.service_stop_summary', { stops: stoppingCount, total: svc.stops.length })}</div>
        </div>`;
      }
      if (!s.services.length) {
        html += `<p class="text-dim">${t('ie.rel.no_services_possible')}</p>`;
      }
    }
    html += `<div class="flex gap-8" style="margin-top:16px">
      <button class="btn" onclick="_relRenderStep(3)">${t('common.back')}</button>
      <button class="btn btn-primary" onclick="_relRenderStep(5)">${t('common.next')}</button>
    </div></div>`;
    el.innerHTML = header(4, t('ie.rel.step_services')) + html;
    return;
  }
  ...
  if (n === 5) {
    ...
    let html = `<div style="margin-top:20px">`;
    if (!w.length) {
      html += `<p class="text-dim">${t('ie.rel.no_warnings')}</p>`;
    } else {
      const grouped = {};
      for (const warn of w) {
    ...
    html += `<div class="flex gap-8" style="margin-top:16px">
      <button class="btn" onclick="_relRenderStep(4)">${t('common.back')}</button>
      <button class="btn btn-primary" onclick="_relRenderStep(6)">${t('common.next')}</button>
    </div></div>`;
    el.innerHTML = header(5, t('ie.rel.step_warnings')) + html;
    return;
  }
  ...
  if (n === 6) {
    ...
    let summaryLines = `<div>${t('ie.rel.importing_stations', { n: stCount })}</div>
          <div>${t('ie.rel.importing_segments', { n: sgCount })}</div>`;
    if (svcCount) summaryLines += `<div>${t('ie.rel.importing_services', { n: svcCount })}</div>`;
    if (s.warnings.length) summaryLines += `<div style="color:var(--warn)">${t('ie.rel.warnings_count', { n: s.warnings.length })}</div>`;

    let manualItems = `<li>${t('ie.rel.manual_junctions')}</li>
            <li>${t('ie.rel.manual_tracks')}</li>
            <li>${t('ie.rel.manual_platforms')}</li>`;
    if (!svcCount) manualItems += `<li>${t('ie.rel.manual_services')}</li>`;
    manualItems += `<li>${t('ie.rel.manual_lines')}</li>`;

    el.innerHTML = header(6, t('ie.rel.step_confirm')) + `
      <div style="margin-top:20px;max-width:500px">
        <div class="ie-card" style="margin-bottom:20px">
          <div style="font-size:16px;font-weight:600;margin-bottom:8px">${t('ie.rel.summary')}</div>
          ${summaryLines}
        </div>
        <div class="ie-card" style="margin-bottom:20px">
          <div style="font-size:13px;font-weight:600;margin-bottom:8px">${t('ie.rel.whats_left')}</div>
          <ul class="text-dim" style="font-size:12px;margin-left:16px;line-height:1.8">
            ${manualItems}
          </ul>
        </div>
        <div class="flex gap-8">
          <button class="btn" onclick="_relRenderStep(5)">${t('common.back')}</button>
          <button class="btn btn-primary" onclick="_relConfirmImport()">${t('ie.csv.btn_import', { n: totalItems })}</button>
        </div>
      </div>`;
    return;
  }
...
async function _relFetchAndProcess() {
  ...
  if (!uniqueIds.length) { toast(t('ie.rel.no_ids'), 'error'); return; }
  ...
  try {
    ...
    for (let ri = 0; ri < uniqueIds.length; ri++) {
      ...
      const label = uniqueIds.length > 1
        ? t('ie.rel.fetching_n', { n: ri + 1, total: uniqueIds.length })
        : t('ie.rel.fetching');
    ...
    toast(t('ie.rel.fetch_error', { msg: err.message }), 'error');
    if (btn) { btn.disabled = false; btn.textContent = t('ie.rel.fetch_btn'); }
  }
}
...
function _relProximityDedup(s) {
  ...
  s.warnings.push({
    ...
    message: t('ie.rel.proximity_merge_summary', {
...
async function _relConfirmImport() {
  ...
  _relImporting = false;
  refreshAll();
  toast(t('ie.csv.import_success', { n: stationsToImport.length + segmentsToImport.length + svcCount }), 'success');
  renderImportExport();
}
...
function _csvStepHeader(stepNum, totalSteps, title) {
  ...
  const modeLabel = s.mode === 'nodes' ? t('ie.csv.mode_nodes') : t('ie.csv.mode_segments');
  return `<div class="csv-wizard-header">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
      <div>
        <h2 style="font-family:var(--font-display);font-size:22px;font-weight:600;margin-bottom:2px">${modeLabel}</h2>
        <div class="text-dim" style="font-size:13px">${t('ie.csv.step_of', { n: stepNum, total: totalSteps })}: ${title}</div>
      </div>
      <button class="btn" onclick="_csvCancel()">${t('common.cancel')}</button>
    </div>
    <div class="csv-step-bar">${Array.from({length: totalSteps}, (_, i) =>
...
function _csvRenderStep(n) {
  ...
  if (n === 1) {
    el.innerHTML = _csvStepHeader(1, totalSteps, t('ie.csv.step_upload')) + `
      <div class="ie-card" style="max-width:500px;margin-top:20px">
        <p style="margin-bottom:16px">${t('ie.csv.upload_desc')}</p>
        <input type="file" accept=".csv,.tsv,.txt" onchange="_csvHandleFile(event)" style="font-size:13px">
        <div style="margin-top:16px;padding-top:16px;border-top:1px solid var(--border)">
          <label style="font-size:12px;font-weight:600;color:var(--text-dim);text-transform:uppercase;letter-spacing:0.04em;display:block;margin-bottom:5px">${t('ie.csv.or_paste')}</label>
          <textarea id="csv-paste-area" placeholder="${t('ie.csv.paste_placeholder')}" style="width:100%;height:120px;font-family:var(--font-mono);font-size:12px;resize:vertical"></textarea>
          <button class="btn" style="margin-top:8px" onclick="_csvHandlePaste()">${t('ie.csv.use_pasted')}</button>
        </div>
      </div>`;
    return;
  }
  ...
  if (n === 2) {
    ...
    const delimLabel = s.file.delimiter === '\t' ? t('ie.csv.delim_tab') : t('ie.csv.delim_comma');
    let table = `<table class="data-table csv-preview-table"><thead><tr>`;
    const colCount = rows[0]?.length || 0;
    for (let c = 0; c < colCount; c++) table += `<th>${t('ie.csv.col')} ${c + 1}</th>`;
    table += `</tr></thead><tbody>`;
    for (let r = 0; r < Math.min(rows.length, 5); r++) {
      table += `<tr>${rows[r].map(v => `<td>${esc(v)}</td>`).join('')}</tr>`;
    }
    table += `</tbody></table>`;
    el.innerHTML = _csvStepHeader(2, totalSteps, t('ie.csv.step_preview')) + `
      <div style="margin-top:20px">
        <div class="text-dim" style="margin-bottom:8px">${t('ie.csv.detected', { delim: delimLabel, rows: rows.length, cols: colCount })}</div>
        <div style="overflow-x:auto">${table}</div>
        <div class="flex gap-8" style="margin-top:16px">
          <button class="btn" onclick="_csvRenderStep(1)">${t('common.back')}</button>
          <button class="btn btn-primary" onclick="_csvRenderStep(3)">${t('common.next')}</button>
        </div>
      </div>`;
    return;
  }
  ...
  if (n === 3) {
    ...
    html += `<p class="text-dim" style="margin-bottom:12px">${t('ie.csv.assign_desc')}</p>`;
    for (let c = 0; c < colCount; c++) {
      const sample = firstRow[c] || '';
      html += `<div class="csv-assign-row">
        <div class="csv-assign-label">${t('ie.csv.col')} ${c + 1}</div>
    ...
    html += `<div class="flex gap-8" style="margin-top:16px">
      <button class="btn" onclick="_csvRenderStep(2)">${t('common.back')}</button>
      <button class="btn btn-primary" onclick="_csvRenderStep(4)">${t('common.next')}</button>
    </div></div>`;
    el.innerHTML = _csvStepHeader(3, totalSteps, t('ie.csv.step_assign')) + html;
    return;
  }
...
function _csvHandlePaste() {
  ...
  if (!raw) { toast(t('ie.csv.paste_empty'), 'error'); return; }
  const parsed = parseCSV(raw);
...
function _csvRenderNodeStep(n, totalSteps) {
  ...
  if (n === 4) {
    ...
    if (!hasTypeCol) {
      html += `<div class="form-group"><label>${t('ie.csv.default_type')}</label>
    ...
    if (!hasPlatCol) {
      html += `<div class="form-group"><label>${t('ie.csv.default_platforms')}</label>
    ...
    html += `<div class="flex gap-8" style="margin-top:16px">
      <button class="btn" onclick="_csvRenderStep(3)">${t('common.back')}</button>
      <button class="btn btn-primary" onclick="_csvBuildNodePreview()">${t('ie.csv.btn_preview')}</button>
    </div></div>`;
    el.innerHTML = _csvStepHeader(4, totalSteps, t('ie.csv.step_defaults')) + html;
    return;
  }
  ...
  if (n === 5) {
    ...
    if (!dupes.length) {
      el.innerHTML = _csvStepHeader(5, totalSteps, t('ie.csv.step_dedup')) + `
        <div style="margin-top:20px">
          <p class="text-dim">${t('ie.csv.no_dupes')}</p>
          <div class="flex gap-8" style="margin-top:16px">
            <button class="btn" onclick="_csvRenderStep(4)">${t('common.back')}</button>
            <button class="btn btn-primary" onclick="_csvRenderStep(6)">${t('common.next')}</button>
          </div>
        </div>`;
      return;
    }
    let html = `<div style="margin-top:20px">
      <p class="text-dim" style="margin-bottom:12px">${t('ie.csv.dedup_node_desc', { n: dupes.length })}</p>
      <div style="overflow-x:auto;max-height:450px;overflow-y:auto">
      <table class="data-table csv-preview-table"><thead><tr>
        <th>${t('ie.csv.importing')}</th><th>${t('ie.csv.existing')}</th><th>${t('ie.csv.conflict')}</th><th>${t('ie.csv.action')}</th>
      </tr></thead><tbody>`;
    for (const { nd, i } of dupes) {
      ...
      html += `<tr>
        ...
        <td>${esc(existName)}</td>
        <td class="text-dim" style="font-size:11px">${nd._dupType === 'ogf' ? t('ie.csv.dup_same_ogf') : t('ie.csv.dup_same_batch')}</td>
        <td style="white-space:nowrap">
          <select onchange="_csvNodeDedupAction(${i},this.value)" class="csv-match-select">
            <option value="skip" ${nd._include === false ? 'selected' : ''}>${t('ie.csv.action_skip')}</option>
            <option value="disambig" ${nd._include !== false ? 'selected' : ''}>${t('ie.csv.action_disambig')}</option>
          </select>
          ${nd._include !== false ? `<input type="text" value="${esc(nd._disambig || '')}" placeholder="${t('ie.csv.disambig_placeholder')}"
    ...
    html += `</tbody></table></div>
      <div class="flex gap-8" style="margin-top:16px">
        <button class="btn" onclick="_csvRenderStep(4)">${t('common.back')}</button>
        <button class="btn btn-primary" onclick="_csvApplyNodeDedup();_csvRenderStep(6)">${t('common.next')}</button>
      </div></div>`;
    el.innerHTML = _csvStepHeader(5, totalSteps, t('ie.csv.step_dedup')) + html;
    return;
  }
  ...
  if (n === 6) {
    ...
    el.innerHTML = _csvStepHeader(6, totalSteps, t('ie.csv.step_review')) + `
      <div style="margin-top:20px">
        <div class="text-dim" style="margin-bottom:8px">${t('ie.csv.review_count', { n: count })}</div>
        <div style="overflow-x:auto;max-height:400px;overflow-y:auto">${table}</div>
        <div class="flex gap-8" style="margin-top:16px">
          <button class="btn" onclick="_csvRenderStep(5)">${t('common.back')}</button>
          <button class="btn btn-primary" onclick="_csvConfirmNodeImport()">${t('ie.csv.btn_import', { n: count })}</button>
        </div>
      </div>`;
    return;
  }
...
function _csvBuildNodePreview() {
  ...
  for (const row of s.file.rows) {
    ...
    if (!name && !ogfNode) { warn = t('ie.csv.warn_no_name_or_ogf'); include = false; }
    else if (!name) warn = t('ie.csv.warn_no_name');
...
async function _csvConfirmNodeImport() {
  ...
  if (!toImport.length) { toast(t('ie.csv.nothing_to_import'), 'error'); _csvImporting = false; return; }
  ...
  if (withOgf.length) {
    toast(t('ie.csv.fetching_ogf', { n: withOgf.length }), 'info');
  ...
  _csvImporting = false;
  refreshAll();
  toast(t('ie.csv.import_success', { n: toImport.length }), 'success');
  renderImportExport();
}
...
function _csvRenderSegStep(n, totalSteps) {
  ...
  if (n === 4) {
    ...
    let html = `<div style="margin-top:20px">
      <p class="text-dim" style="margin-bottom:12px">${t('ie.csv.match_desc')}</p>
      <div style="overflow-x:auto;max-height:450px;overflow-y:auto">
      <table class="data-table csv-preview-table"><thead><tr>
        <th>${t('ie.csv.row')}</th><th>${t('ie.csv.from_input')}</th><th>${t('ie.csv.from_match')}</th>
        <th>${t('ie.csv.to_input')}</th><th>${t('ie.csv.to_match')}</th></tr></thead><tbody>`;
    ...
    html += `</tbody></table></div>
      <div class="flex gap-8" style="margin-top:16px">
        <button class="btn" onclick="_csvRenderStep(3)">${t('common.back')}</button>
        <button class="btn btn-primary" onclick="_csvRenderStep(5)">${t('common.next')}</button>
      </div></div>`;
    el.innerHTML = _csvStepHeader(4, totalSteps, t('ie.csv.step_matching')) + html;
    return;
  }
  ...
  if (n === 5) {
    ...
    if (!hasSpeed) {
      html += `<div class="form-group"><label>${t('ie.csv.default_speed')}</label>
    ...
    if (!hasTracks) {
      html += `<div class="form-group"><label>${t('ie.csv.default_tracks')}</label>
    ...
    if (!hasElec) {
      html += `<div class="form-group"><label>${t('ie.csv.default_elec')}</label>
    ...
    html += `<div class="flex gap-8" style="margin-top:16px">
      <button class="btn" onclick="_csvRenderStep(4)">${t('common.back')}</button>
      <button class="btn btn-primary" onclick="_csvBuildSegPreview()">${t('ie.csv.btn_preview')}</button>
    </div></div>`;
    el.innerHTML = _csvStepHeader(5, totalSteps, t('ie.csv.step_defaults')) + html;
    return;
  }
  ...
  if (n === 6) {
    ...
    if (!dupes.length) {
      el.innerHTML = _csvStepHeader(6, totalSteps, t('ie.csv.step_dedup')) + `
        <div style="margin-top:20px">
          <p class="text-dim">${t('ie.csv.no_dupes')}</p>
          <div class="flex gap-8" style="margin-top:16px">
            <button class="btn" onclick="_csvRenderStep(5)">${t('common.back')}</button>
            <button class="btn btn-primary" onclick="_csvRenderStep(7)">${t('common.next')}</button>
          </div>
        </div>`;
      return;
    }
    let html = `<div style="margin-top:20px">
      <p class="text-dim" style="margin-bottom:12px">${t('ie.csv.dedup_seg_desc', { n: dupes.length })}</p>
      <div style="overflow-x:auto;max-height:450px;overflow-y:auto">
      <table class="data-table csv-preview-table"><thead><tr>
        <th>${t('ie.csv.importing')}</th><th>${t('ie.csv.conflict')}</th><th>${t('ie.csv.action')}</th>
      </tr></thead><tbody>`;
    for (const { seg, i } of dupes) {
      ...
      const typeLabel = seg._dupType === 'pair' ? t('ie.csv.dup_same_endpoints')
        : seg._dupType === 'ways' ? t('ie.csv.dup_same_ways')
        : t('ie.csv.dup_same_batch');
      html += `<tr>
        ...
        <td>
          <select onchange="_csvImportState.preview[${i}]._include=this.value==='keep'" class="csv-match-select">
            <option value="skip" ${seg._include === false ? 'selected' : ''}>${t('ie.csv.action_skip')}</option>
            <option value="keep" ${seg._include !== false ? 'selected' : ''}>${t('ie.csv.action_keep')}</option>
          </select>
        </td>
      </tr>`;
    }
    ...
    el.innerHTML = _csvStepHeader(6, totalSteps, t('ie.csv.step_dedup')) + html;
    return;
  }
  ...
  if (n === 7) {
    ...
    el.innerHTML = _csvStepHeader(7, totalSteps, t('ie.csv.step_review')) + `
      <div style="margin-top:20px">
        <div class="text-dim" style="margin-bottom:8px">${t('ie.csv.review_count', { n: count })}</div>
        <div style="overflow-x:auto;max-height:400px;overflow-y:auto">${table}</div>
        <div class="flex gap-8" style="margin-top:16px">
          <button class="btn" onclick="_csvRenderStep(6)">${t('common.back')}</button>
          <button class="btn btn-primary" onclick="_csvConfirmSegImport()">${t('ie.csv.btn_import', { n: count })}</button>
        </div>
      </div>`;
    return;
  }
...
function _csvBuildSegNodeMatches() {
  ...
  if (!s.nodeMatches || !Object.keys(s.nodeMatches).length) {
    ...
    for (let i = 0; i < s.file.rows.length; i++) {
      ...
      const fromWarn = fromRes.mode === 'ogf' && fromRes.matches.length > 1 ? t('ie.csv.warn_dup_ogf_match') : '';
      const toWarn = toRes.mode === 'ogf' && toRes.matches.length > 1 ? t('ie.csv.warn_dup_ogf_match') : '';
...
function _csvMatchSelect(rowIdx, direction, matches) {
  ...
  if (!matches.length) return `<span class="csv-match-none">${t('ie.csv.no_match')}</span>`;
  let html = `<select onchange="_csvImportState.nodeMatches[${rowIdx}].${direction}Selected=this.value||null" class="csv-match-select">`;
  html += `<option value="">${t('ie.csv.select_node')}</option>`;
...
function _csvBuildSegPreview() {
  ...
  for (let i = 0; i < s.file.rows.length; i++) {
    ...
    if (!fromId && !toId) { warn = t('ie.csv.warn_no_nodes'); include = false; }
    else if (!fromId) { warn = t('ie.csv.warn_no_from'); include = false; }
    else if (!toId) { warn = t('ie.csv.warn_no_to'); include = false; }
    else if (fromId === toId) { warn = t('ie.csv.warn_same_node'); include = false; }
...
async function _csvConfirmSegImport() {
  ...
  if (withWays.length) {
    toast(t('ie.csv.fetching_ways', { n: withWays.length }), 'info');
    try {
